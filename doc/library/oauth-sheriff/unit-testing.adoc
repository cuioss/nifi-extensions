...
This document describes the test utilities provided by the `oauth-sheriff-core` project for testing JWT validation functionality in your applications.
...
* `de.cuioss.sheriff.oauth.core.test.dispatcher` - Mock dispatchers for JWKS resolution
* `de.cuioss.sheriff.oauth.core.test.generator` - Token and claim generators
* `de.cuioss.sheriff.oauth.core.test.util` - Utility classes for testing
...
void shouldValidateToken() {
    ...
    // Create a TokenValidator with a test issuer configuration
    TokenValidator validator = TokenValidator.builder()
            .issuerConfig(testIssuerConfig)
            .build();

    // Act - createAccessToken throws TokenValidationException if validation fails
    AccessTokenContent accessToken = validator.createAccessToken(token);
...
void shouldRejectTamperedToken() {
    ...
    // Act & Assert - tampered token should cause TokenValidationException
