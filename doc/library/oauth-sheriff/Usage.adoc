...
This document provides usage examples for the OAuth-Sheriff library. For detailed technical specifications, see the link:../../specification/configuration.adoc[Configuration Reference].
...
_See link:../../specification/configuration.adoc#_controller_service_architecture[Controller Service Architecture]_
...
// Create a validator with the issuer configuration
...
_See link:../../specification/configuration.adoc[Configuration Reference] for details_
...
ParserConfig config = ParserConfig.builder()
        ...
        .maxStringLength(2 * 1024)     // Limit JSON string length to 2KB (default is 4KB)
        .maxBufferSize(64 * 1024)      // Limit JSON buffer size to 64KB (default is 128KB)
        .build();
...
* `maxStringLength`, `maxBufferSize`: Prevent JSON parsing attacks
...
try {
    ...
    // Note: getSubject() returns Optional<String>
    accessToken.getSubject().ifPresent(
            subject -> LOGGER.info("Token validated successfully for subject: %s", subject));
...
For a complete list of event types and categories, refer to the `SecurityEventCounter.EventType` enum and the `EventCategory` enum.
