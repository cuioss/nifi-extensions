...
7. Created token generation utilities using Keycloak API as a Cypress custom command `cy.getAccessToken` in `nifi-cuioss-ui/cypress/support/commands.js`. Keycloak connection details were added to `cypress.config.js`.
...
1. Implemented Cypress custom commands for login (`cy.login`) and navigation (`cy.navigateToControllerServices`, `cy.navigateToProcessGroup`, `cy.navigateToProcessorConfiguration`) in `nifi-cuioss-ui/cypress/support/commands.js`. The `cy.login` command uses JWTs obtained via `cy.getAccessToken` and stored in `localStorage`. Leveraged `NifiCanvasPage` page object for some operations. Added `nifiUrl` to `cypress.config.js`.
2. Created basic processor configuration tests in `nifi-cuioss-ui/cypress/integration/configuration/basic-configuration.spec.js`. These tests cover adding the processor, configuring basic (Token Location, Header Name) and advanced (Max Token Size, Refresh Interval) properties, saving, and verifying persistence. Enhanced `NifiCanvasPage` and `ProcessorConfigurationPage` page objects and included test cleanup via `afterEach`.
...
[end of doc/implement-end-to-end.adoc]
