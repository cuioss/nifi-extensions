...
`
. Access NiFi UI at https://localhost:9095/nifi/ (credentials: admin/adminadminadmin)

== Adding the Processor to the Canvas

. Log in to the NiFi UI using the credentials: admin/adminadminadmin
...
The same `JwtIssuerConfigService` Controller Service configured here is also used by the RestApiGatewayProcessor. If you configure issuers for the JWT processor, the gateway can reuse the same Controller Service instance. See the link:QuickStart.adoc#_restapigateay_quick_start[RestApiGateway Quick Start] for gateway-specific configuration.