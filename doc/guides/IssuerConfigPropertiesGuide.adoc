...
* Select the JWKS Type (URL, File, or In-Memory) — this controls which JWKS input field is shown
* Fill in the fields:
** Issuer URI (optional): The URI of the token issuer (must match the "iss" claim in the JWT). At least one of Issuer URI or Name is required.
...
** JWKS Content (when type is In-Memory): Inline JWKS JSON content
