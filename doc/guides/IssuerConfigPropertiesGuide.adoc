...
* `issuer.keycloak.issuer`: Issuer URI (must match the `iss` claim in the JWT)
** Example: `https://keycloak:8443/realms/oauth_integration_tests`
...
* `issuer.keycloak.name` (optional): Alternative issuer identifier; if both `name` and `issuer` are set, `name` takes precedence
