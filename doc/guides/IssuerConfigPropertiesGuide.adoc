...
* NiFi test environment set up (see <<_test_environment_setup,Test Environment Setup>>)
...
** Require Valid Token: true (default) — controls missing-token behavior only; invalid tokens always route to `authentication-failed`
** Required Roles: (optional) comma-separated list of roles the token must contain
** Required Scopes: (optional) comma-separated list of scopes the token must contain
...
* Select the JWKS Source Type (URL or File) — this controls which JWKS input field is shown. Note: the UI also shows a "Memory" option, but inline JWKS content is not yet supported by the backend and will cause the issuer to be skipped.
* Fill in the fields:
** Issuer URI (required): The URI of the token issuer (must match the "iss" claim in the JWT)
** Example: `https://keycloak:8443/realms/oauth_integration_tests`
** JWKS URL (when type is URL): The URL of the JWKS endpoint
** Example: `https://keycloak:8443/realms/oauth_integration_tests/protocol/openid-connect/certs`
** JWKS File (when type is File): Path to a local JWKS key file
...
If the custom tab is not visible or not functioning properly, you can configure issuers using dynamic properties on the **Controller Service**:

. Open the Controller Service configuration, in the "Properties" tab click the "+" button to add a new property
. Add properties in the format `issuer.<issuer-name>.<property-key>`:
* `issuer.keycloak.issuer` (required): Issuer URI, must match the `iss` claim in the JWT -- this is the sole source of issuer identity, and an issuer without it is skipped
** Example: `https://keycloak:8443/realms/oauth_integration_tests`
* `issuer.keycloak.jwks-url`: JWKS endpoint URL
** Example: `https://keycloak:8443/realms/oauth_integration_tests/protocol/openid-connect/certs`
* `issuer.keycloak.audience` (optional): Required `aud` claim value
* `issuer.keycloak.client-id` (optional): Expected client ID claim
* `issuer.keycloak.enabled` (optional): Set to `false` to disable without removing
* `issuer.keycloak.name` (optional): Human-readable label only -- accepted and ignored, it never contributes to issuer identity
* `issuer.keycloak.jwks-file` (optional): File path to a local JWKS key file (alternative to `jwks-url`)
* `issuer.keycloak.jwks-type` (optional): Explicit JWKS source type (`url` or `file`; auto-detected if omitted)
+
See link:../reference/configuration.adoc#_dynamic_properties_issuers[Configuration Reference — Dynamic Properties] for the complete property list.
...
If the "Configuration" tab is not visible:

. Check browser console for errors (F12 > Console)
. Verify that the custom UI components are loaded:
* Check for JavaScript or CSS loading errors in the browser console
* Ensure the NiFi extensions NAR is properly deployed
...
Refer to the link:../reference/configuration.adoc#_static_configuration_files[Static Configuration Files] and link:../reference/configuration.adoc#_environment_variables[Environment Variables] sections in the Configuration Reference for details.

=== Using with RestApiGatewayProcessor

The same `JwtIssuerConfigService` Controller Service configured here is also used by the RestApiGatewayProcessor. If you configure issuers for the JWT processor, the gateway can reuse the same Controller Service instance. The gateway also provides its own **Issuer Configuration** tab for managing issuers directly from the gateway UI. See the link:QuickStart.adoc[Quick Start Guide] for gateway-specific configuration.