...
|`false`
|Yes
|Whether to allow JWKS URLs pointing to private network addresses. Enable only for development or trusted environments (e.g., Docker/Kubernetes deployments where the IdP runs on an internal network).
|===
...
|`issuer.<name>.name`
|Issuer identifier name (alternative to `issuer.<name>.issuer`); at least one of `name` or `issuer` is required. *Warning*: This value becomes the `issuerIdentifier` used for token matching. If both `name` and `issuer` are set, `name` takes precedence. Use a URI matching the `iss` claim unless you have a specific reason not to.
