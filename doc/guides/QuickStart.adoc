...
The `RestApiGatewayProcessor` provides a self-contained REST API gateway with embedded HTTP server.
...
=== Step 4: Add Routes via Dynamic Properties

Click the "+" button to add dynamic properties for your routes:
...
Start the processor. The gateway listens on the configured port. Test with:
...
A successful request returns HTTP 202 (Accepted) and creates a FlowFile on the `users` relationship.
