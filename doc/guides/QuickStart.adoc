...
* Both `nifi-cuioss-api-nar-<version>.nar` and `nifi-cuioss-nar-<version>.nar` deployed to `$NIFI_HOME/extensions/` (see link:../../README.adoc#_installation[Installation])
...
* **Standard Endpoints**: built-in endpoints (`/health`, `/metrics`, `/status/{traceId}`, `/attachments/{parentTraceId}`) with configurable auth modes and enabled status
...
The route editor form is organized into sections:
...
|TRACKING
|Tracking Mode
|`none` (default), `simple`, or `attachments` for async request tracking

|TRACKING
|Attachments Min/Max Count
|Minimum and maximum attachments (only for `attachments` mode)

|TRACKING
|Attachments Timeout
|Collection timeout (only for `attachments` mode)
|===
...
. Paste an authorization token (or leave empty to test unauthenticated access). Alternatively, expand the **Fetch Token** section to acquire a token from a configured issuer's token endpoint.
