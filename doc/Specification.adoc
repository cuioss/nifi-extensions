...
The MultiIssuerJWTTokenAuthenticator is compatible with NiFi 2.6.0 and leverages the oauth-sheriff-core library for token validation.
...
* xref:specification/configuration-ui.adoc[UI Configuration] - UI configuration details
...
These libraries provide a robust foundation for implementing and testing JWT authentication in NiFi 2.6.0 environments.
...
The processor validates JWT tokens using the oauth-sheriff-core library with:
...
=== NiFi 2.6.0 Compatibility

The processor is designed specifically for NiFi 2.6.0 and takes advantage of its improved:
...
=== oauth-sheriff-core Integration

The processor fully leverages the oauth-sheriff-core library's capabilities:
