...
* **RestApiGatewayProcessor** -- HTTP server settings, route configuration, management endpoints, and CS reference
...
|true
|Whether the built-in `/health` management endpoint is active
|No

|rest.gateway.management.health.auth-mode
|local-only
|Authentication mode for `/health`: `bearer` (JWT required), `none` (anonymous), `local-only` (loopback bypass; remote needs JWT)
|No

|rest.gateway.management.metrics.enabled
|true
|Whether the built-in `/metrics` management endpoint is active
|No
...
|restapi.<name>.auth-mode
|Authentication mode: `bearer` (JWT required, default), `none` (anonymous), `local-only` (loopback bypass; remote needs JWT)
|none

|restapi.<name>.max-request-size
|Per-route maximum request body size in bytes. When absent or `0`, the global `rest.gateway.max.request.size` applies.
|524288

|restapi.<name>.enabled
|Whether the route is active (default: true). Disabled routes return 404.
|false

|restapi.<name>.schema
|File path to a JSON Schema for request body validation
...
rest.gateway.management.health.auth-mode = local-only
rest.gateway.management.metrics.auth-mode = local-only
...
=== Core Documentation
* link:../../README.adoc[Project Documentation]
