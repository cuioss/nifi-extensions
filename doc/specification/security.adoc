= NiFi Extensions Security Specification
:toc:
:toclevels: 3
...
* link:../../nifi-cuioss-ui/src/main/java/de/cuioss/nifi/ui/util/ComponentConfigReader.java[ComponentConfigReader] -- Secure processor configuration retrieval via NiFi REST API
...
* link:../../nifi-cuioss-ui/src/test/java/de/cuioss/nifi/ui/util/ComponentConfigReaderTest.java[ComponentConfigReaderTest]
...
1. **Lookup only**: The header value is passed to `NiFiWebConfigurationContext` to read the referenced processor's existing, admin-configured properties. An attacker who supplies an arbitrary UUID can only reference components that a NiFi administrator has already created.
2. **No privilege escalation**: The `allowPrivateAddresses` flag is read from a `StandardJwtIssuerConfigService` controller service that the administrator has explicitly configured. The header cannot inject new configuration values.
...
4. **NiFi session authentication**: The Custom UI servlet endpoints are only reachable within NiFi's authenticated web container (iframe). Unauthenticated callers cannot reach the validation endpoints at all.
...
For more details on multi-issuer implementation, see link:configuration.adoc#_controller_service_architecture[Controller Service Architecture].
