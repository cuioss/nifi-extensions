= NiFi Extensions UI Configuration
:toc:
:toclevels: 3
...
This document covers the user interface elements and interaction patterns for configuring both the MultiIssuerJWTTokenAuthenticator and RestApiGatewayProcessor. For general configuration properties and options, see xref:configuration.adoc[Configuration].

=== Unified Custom UI Architecture

The Custom UI WAR (`nifi-cuioss-ui`) serves both processors through a component type detection mechanism:

1. On load, the UI calls the `ComponentInfoServlet` to determine the processor type
2. If the processor class name contains `RestApiGateway`, the UI switches to gateway mode
3. Tab visibility is toggled: JWT tabs are hidden and gateway tabs are shown (or vice versa)

See link:../../nifi-cuioss-ui/src/main/webapp/js/app.js[app.js] for the `configureTabsForType()` implementation.

=== JWT Processor Tabs

* **Configuration** -- manage issuer configurations (add, edit, delete, validate JWKS)
* **Token Verification** -- paste and verify JWT tokens against current configuration
* **Metrics** -- displays "Metrics Not Available" notice (metrics are only available for the gateway processor)
* **Help** -- processor documentation and troubleshooting

=== Gateway Processor Tabs

* **Endpoint Configuration** -- view and manage REST endpoint route configurations
* **Endpoint Tester** -- test gateway endpoints with sample requests
* **Issuer Configuration** -- manage JWT issuer settings (add, edit, delete) directly from the gateway UI, reading from the linked `JwtIssuerConfigService`
* **Token Verification** -- paste and verify JWT tokens against configured issuers (same functionality as the JWT processor tab)
* **Metrics** -- display gateway metrics (token validation, transport security, application events) proxied via `GatewayProxyServlet`
* **Help** -- processor documentation and troubleshooting

=== Gateway UI Sub-Endpoints

The `GatewayProxyServlet` proxies requests from the Custom UI to the running gateway:

* `/api/gateway/config` -- retrieves current route configuration
* `/api/gateway/metrics` -- retrieves metrics from the gateway's `/metrics` endpoint
* `/api/gateway/test` -- sends test requests to the gateway
...
definitions and tracks persisted vs session-only changes with visual origin badges.
...
Location: `src/main/resources/i18n/[bundlename].properties`
...
4. Run verification tests to ensure all bundles contain the required keys
