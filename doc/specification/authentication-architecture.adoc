...
* **Direct path** (left): External client -> Gateway Jetty (single-layer: JWT Bearer or optional API key for `/metrics`)
...
|Per-route config
|Full `cui-http` security pipeline (input sanitization, rate limiting via queue backpressure)
