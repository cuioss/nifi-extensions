...
* **Management endpoints** (`/metrics`, `/config`) -- Optional API key authentication (see <<management-endpoint-security>>).
...
|`/nifi-api/processors/jwt/gateway/*`
|ProcessorIdValidationFilter
|Yes (ComponentConfigReader)
|GET, POST
|Proxies to gateway + SSRF protection
...
|None
|GET
|Bypasses JWT auth and security pipelines -- exposes route configuration
|===
...
Management endpoints (`/metrics`, `/config`) support optional API key authentication via the `rest.gateway.management.api-key` processor property:
...
* **Default (`0.0.0.0`)**: Listens on all interfaces. Management endpoints are network-accessible.
...
The `GatewayProxyServlet` provides authenticated proxy access to management endpoints:
...
4. Reads the management API key from processor config and forwards it via `X-Api-Key` header
5. SSRF protection: only localhost targets allowed

This means users accessing management data through the NiFi UI get full three-layer authentication, even when the gateway's own management endpoints use only API key auth.
...
|Authenticated proxy to gateway management API with SSRF protection
...
|Management endpoint handler with optional API key authentication
