= NiFi Extensions Token Validation
:toc:
:toclevels: 3
...
The processor reads the raw JWT token from a configurable FlowFile attribute (default: `jwt.token`). The attribute name is configured via the `Token Attribute` property.

Token reading is implemented in link:../../nifi-cuioss-processors/src/main/java/de/cuioss/nifi/processors/auth/MultiIssuerJWTTokenAuthenticator.java[MultiIssuerJWTTokenAuthenticator] (`onTrigger` method), which reads the token via `flowFile.getAttribute(tokenAttributeName)`.

For more information about token attribute configuration, see link:configuration.adoc[Configuration].
...
For detailed implementation of this lifecycle management, see link:configuration.adoc#_controller_service_architecture[Controller Service Architecture].
...
Flow file attribute generation is implemented in link:../../nifi-cuioss-processors/src/main/java/de/cuioss/nifi/processors/auth/MultiIssuerJWTTokenAuthenticator.java[MultiIssuerJWTTokenAuthenticator] (`onTrigger` method), using attribute key constants from link:../../nifi-cuioss-common/src/main/java/de/cuioss/nifi/jwt/JWTAttributes.java[JWTAttributes].
