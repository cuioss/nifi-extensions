...
The processor uses the TokenFactory from portal-authentication-token to validate token signatures. The implementation follows a simple two-step approach:

1. Get the token parser from the current configuration
2. Use TokenFactory to create and validate the access token

[source,java]
----
/**
 * Validates a token using TokenFactory.
 */
private Optional<ParsedAccessToken> validateToken(String tokenString) {
    // Get the token parser from configuration
    JwksAwareTokenParser tokenParser = getConfiguredTokenParser();
 ...
 */
private JwksAwareTokenParser getConfiguredTokenParser() {
    // Get dynamic properties for issuers
    Map<String, String> dynamicProperties = getDynamicProperties();
...
=== Implementation in onTrigger
_See Requirement link:../requirements.adoc#NIFI-AUTH-3[NIFI-AUTH-3: Token Validation Requirements]_

The token validation is implemented in the onTrigger method:
...
public void onTrigger(final ProcessContext context, final ProcessSession session) {
    ...
    try {
        ...
        // Validate token
        Optional<ParsedAccessToken> accessToken = validateToken(token);
