...
public TokenVerificationResult verifyTokenForUi(String tokenString, List<IssuerConfig> issuerConfigs) {
    ...
    Optional<AccessTokenContent> result = validator.createAccessToken(tokenString);

    // 4. Prepare the response for the UI
    if (result.isPresent()) {
        // Token is valid
        return TokenVerificationResult.success(result.get(), eventCounter);
    } else {
        // Token is invalid, get error details from the counter
        return TokenVerificationResult.failure(eventCounter.getLastError(), eventCounter);
    }
}
----
