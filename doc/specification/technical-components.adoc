...
This document describes the technical components of the MultiIssuerJWTTokenAuthenticator processor, their responsibilities, and how they interact to provide JWT token validation capabilities in NiFi. The processor is designed for NiFi 2.4.0 and leverages the oauth-sheriff-core library for token validation functionality.
...
|de.cuioss.portal.authentication:oauth-sheriff-core
...
For detailed implementation of token validation, see link:library/oauth-sheriff-core/Usage.adoc[Token Validation Specification].
...
For more configuration details, see link:library/oauth-sheriff-core/Usage.adoc[Configuration Specification].
...
The oauth-sheriff-core library's `TokenValidator` is designed to be stateful and maintain caches for performance optimization. In the MultiIssuerJWTTokenAuthenticator processor, the TokenValidator instance is managed with careful attention to NiFi's processor lifecycle.

For complete details on TokenValidator lifecycle management, including thread safety and resource management, see link:library/oauth-sheriff-core/Usage.adoc#handling-tokenvalidator-lifecycle-in-nifi[Handling TokenValidator Lifecycle in NiFi].
...
For detailed information about attribute handling and token validation output, see link:library/oauth-sheriff-core/Usage.adoc#flow-file-attribute-generation[Flow File Attribute Generation].

== Error Handling

The processor implements comprehensive error handling using standardized error codes and consistent messages. For complete details on error handling strategies and implementation, see link:library/oauth-sheriff-core/unit-testing.adoc[Error Handling Specification].

== Security Considerations

Security is paramount in JWT token validation. For information about security implementation, best practices, and threat mitigations, see link:library/oauth-sheriff-core/Usage.adoc[Security Specification].
...
=== Implementation
* link:library/oauth-sheriff-core/Usage.adoc[Token Validation]
* link:library/oauth-sheriff-core/unit-testing.adoc[Error Handling]
* link:library/oauth-sheriff-core/Usage.adoc[Security]
