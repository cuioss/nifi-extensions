...
The processor leverages the SecurityEventCounter from the oauth-sheriff-core library to track key security events:
...
The processor collects metrics using the SecurityEventCounter provided by the oauth-sheriff-core library. See link:../../nifi-cuioss-processors/src/main/java/de/cuioss/nifi/processors/auth/MultiIssuerJWTTokenAuthenticator.java[MultiIssuerJWTTokenAuthenticator] for the metrics collection implementation.
...
The processor integrates with the oauth-sheriff-core SecurityEventCounter as follows:
