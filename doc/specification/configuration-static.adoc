= NiFi Extensions Static Configuration
:toc: left
:toclevels: 3
...
_See Requirement xref:../Requirements.adoc#NIFI-AUTH-7.6[NIFI-AUTH-7.6: Static Configuration for Container Environments]_
...
* link:../../nifi-cuioss-common/src/main/java/de/cuioss/nifi/jwt/config/ConfigurationManager.java[ConfigurationManager] -- Static config loading from files and environment variables
* link:../../nifi-cuioss-common/src/main/java/de/cuioss/nifi/jwt/config/IssuerConfigurationParser.java[IssuerConfigurationParser] -- Parsing issuer configurations from properties and YAML

=== Verification
* link:../../nifi-cuioss-common/src/test/java/de/cuioss/nifi/jwt/config/ConfigurationManagerTest.java[ConfigurationManagerTest]
* link:../../nifi-cuioss-common/src/test/java/de/cuioss/nifi/jwt/config/IssuerConfigurationParserTest.java[IssuerConfigurationParserTest]
...
[NOTE]
====
The processor fully supports both properties and YAML file formats.
====
...
The processor looks for configuration files at the following default paths in the NiFi installation directory:

* `conf/cui-nifi-extensions.properties`
* `conf/cui-nifi-extensions.yml`

The properties file is checked first. If neither file exists, no file-based configuration is loaded.

== Dynamic Configuration Loading

=== Configuration Refresh Behavior

See the configuration refresh logic in link:../../nifi-cuioss-common/src/main/java/de/cuioss/nifi/jwt/config/ConfigurationManager.java[ConfigurationManager].
...
services:
  nifi:
    ...
    ports:
      - "8443:8443"
    volumes:
      - ./config/issuers.yaml:/opt/nifi/nifi-current/conf/cui-nifi-extensions.yml:ro
    environment:
...
The processor implements robust error handling for configuration loading. See link:../../nifi-cuioss-common/src/main/java/de/cuioss/nifi/jwt/config/ConfigurationManager.java[ConfigurationManager] for error handling implementation.
...
2. If no valid configuration has been loaded, flowfiles are routed to the `authentication-failed` relationship
...
* xref:security-architecture.adoc[Security Architecture]
