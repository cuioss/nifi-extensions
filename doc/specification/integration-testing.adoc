...
* link:../../integration-testing/src/test/java/de/cuioss/nifi/integration/CustomUIEndpointsIT.java[CustomUIEndpointsIT] -- Custom UI endpoint tests (port 9095)
* link:../../integration-testing/src/test/java/de/cuioss/nifi/integration/KeycloakRealmConfigIT.java[KeycloakRealmConfigIT] -- Keycloak realm configuration tests (port 9080)
...
* link:../../integration-testing/src/test/resources/junit-platform.properties[junit-platform.properties] -- JUnit 5 parallel execution configuration
...
│  │  Parallel class execution (3 threads)                   │ │
│  │  - NiFiFlowPipelineIT      - CustomUIEndpointsIT       │ │
│  │  - RestApiGatewayIT         - NiFiProcessorMetricsIT    │ │
│  │  - KeycloakRealmConfigIT                                │ │
│  └─────────────────────────────────────────────────────────┘ │
└──────────────────────────────────────────────────────────────┘
----
...
The `@BeforeAll` method `waitForFlowEndpoint()` polls the flow endpoint with a 120-second timeout, retrying every 1 second until a connection is accepted (any HTTP status code, including 401, indicates readiness).
...
The `@BeforeAll` method `waitForGateway()` polls the `/api/health` endpoint with a 120-second timeout, retrying every 1 second until the gateway accepts connections.
...
**File:** link:../../integration-testing/src/test/java/de/cuioss/nifi/integration/CustomUIEndpointsIT.java[CustomUIEndpointsIT.java]

Integration tests for the Custom UI WAR endpoints deployed inside NiFi. Verifies that JWT verification, JWKS validation, component info, and gateway proxy servlets are accessible and respond correctly when called with NiFi authentication and the required `X-Processor-Id` header.

==== Setup

The `@BeforeAll` method discovers the processor IDs dynamically via the NiFi REST API, builds authenticated REST Assured specs, and waits for both the NiFi UI and the REST API Gateway's embedded Jetty to become available (120-second timeout each, retrying every 1 second).
...
4+h|Endpoint Accessibility

|`verifyTokenWithValidKeycloakToken`
|POST real Keycloak JWT to verify-token
|200, valid=true
|Token verification with real JWT

|`verifyTokenWithInvalidToken`
|POST invalid token string
|200, valid=false
|Invalid tokens must not be reported as valid

|`verifyTokenWithMissingToken`
|POST body missing `token` field
|400
|Input validation

|`jwksContentValidationWithEmptyKeys`
|POST JWKS with empty keys array
|valid=false
|Empty JWKS is invalid

|`jwksContentValidationWithValidKeys`
|POST real Keycloak JWKS content
|valid=true, keyCount>0
|JWKS content validation works

|`jwksUrlValidationWithKeycloakUrl`
|POST Keycloak JWKS URL (Docker network)
|valid=true
|SSRF allow-private-addresses setting works

|`jwksFileValidationWithValidFile`
|POST path to test-jwks.json in NiFi conf
|valid=true
|File-based JWKS validation works

4+h|Security

|`missingProcessorIdReturns401`
|Request without X-Processor-Id header
|401
|Processor ID enforcement

|`invalidProcessorIdReturns401`
|X-Processor-Id with non-UUID value
|401
|UUID format validation

4+h|Component Info

|`componentInfoReturnsProcessorType`
|GET component-info for JWT authenticator
|type=PROCESSOR
|Component discovery works

|`componentInfoReturnsGatewayType`
|GET component-info for gateway processor
|type=PROCESSOR
|Gateway component discovery works

4+h|Gateway Proxy Endpoints

|`gatewayConfigReturnsJson`
|GET gateway config via Custom UI proxy
|200, routes present
|Gateway configuration accessible

|`gatewayTestHappyPath`
|POST test request to gateway proxy
|200
|Gateway proxy forwards correctly
|===

=== KeycloakRealmConfigIT

**File:** link:../../integration-testing/src/test/java/de/cuioss/nifi/integration/KeycloakRealmConfigIT.java[KeycloakRealmConfigIT.java]

Validates that the Keycloak realm configurations (`oauth_integration_tests` and `other_realm`) are imported correctly and produce JWTs with the expected claims, roles, and issuer values.
Does NOT test generic Keycloak behavior — instead validates the specific realm setup that downstream processors depend on.

==== Setup

The `@BeforeAll` method waits for the Keycloak endpoint (120-second timeout, retrying every 1 second), then builds REST Assured specs for both realms.
...
4+h|Roles Claim Mapper

|`testUserShouldHaveUserAndReadRoles`
|Decode testUser token
|`roles` claim contains `user` and `read`
|Realm-roles-mapper emits roles into tokens

|`limitedUserShouldHaveUserRoleOnly`
|Decode limitedUser token
|`roles` contains `user`, NOT `read`
|Role-based access control differentiation

4+h|Issuer Configuration

|`primaryRealmIssuerShouldContainRealmName`
|Decode testUser token
|`iss` contains `oauth_integration_tests`
|Correct issuer for primary realm

|`otherRealmIssuerShouldDiffer`
|Compare issuers from both realms
|Different `iss` values
|Realms are distinct

4+h|Signing Key Isolation

|`realmsShouldUseDifferentSigningKeys`
|Compare `kid` from both realm tokens
|Different `kid` values
|Cross-realm tokens are rejected by signature

4+h|Client Configuration

|`testClientShouldSupportPasswordGrant`
|Password grant with valid credentials
|200, access_token present
|Direct access grants enabled

|`testClientShouldRejectClientCredentialsGrant`
|Client credentials grant
|401
|Only password grant is allowed
|===
...
|`concurrent` — different IT classes run in parallel

|Method-level parallelism
|`same_thread` — methods within a class run sequentially (preserves `@Nested` ordering)

|Thread count
|3 fixed threads (sufficient for 5 classes, avoids overloading containers)
|===

All tests are read-only against the shared containers (no state mutations), making parallel class execution safe.

=== NAR Build Optimization

The `copy-deployment.sh` script supports a `--skip-build` flag:

* **Maven path** (`deploy-and-start.sh` in `pre-integration-test` phase): passes `--skip-build` because the Maven reactor (`-am`) already built all NARs in the `package` phase. This avoids a redundant 30-60 second Maven invocation.
* **Standalone path** (`run-and-deploy.sh`, `redeploy-nifi.sh`): does NOT pass the flag, so NARs are always rebuilt to reflect any code changes.
* **Safety net**: even with `--skip-build`, if NAR files are missing (e.g., after `mvnw clean`), the Maven build runs anyway.

=== Container Health Check Tuning

Keycloak uses aggressive health check intervals to minimize startup wait time:
...
The `wait-for-containers.sh` script polls in three stages:

1. **Container running** — 5-second intervals
2. **NiFi UI accessible** — continuous (part of stage 1 loop)
3. **Flow pipeline on port 7777** — 2-second intervals
