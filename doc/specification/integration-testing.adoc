...
Tests are activated via the `local-integration-tests` Maven profile and require `integration.test.local=true`.
They are _not_ run in CI because they require running Docker containers.
