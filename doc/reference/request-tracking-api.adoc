...
IMPORTANT: A malformed `errorStatus` (not an integer, or outside the `100`-`599` range) or `errorViolations` (not a serialized JSON array) omits *only that one response member*, without failing the response.
