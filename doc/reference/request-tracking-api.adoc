...
It is emitted whenever *any* `error*` cache field is populated, regardless of the entry's `status` — `REJECTED` and `ERROR` are the common cases, but a `PROCESSED` entry carrying a warning detail returns the object just the same.
It is absent only when no `error*` field is populated.
...
Six optional cache fields populate the response's `error` object.
...
|`errorStatus`
|`error.status` (**number**)
|Written as a *String holding an integer*; the gateway parses it into a JSON number. No `100`-`599` range check is applied — any parseable integer is passed through.
...
IMPORTANT: A malformed `errorStatus` (not an integer) or `errorViolations` (not a serialized JSON array) silently omits *only that one response member*.
The rest of the `error` object is still returned, the request never fails, and the gateway records a WARN in the NiFi log — `REST-125` for `errorStatus`, `REST-126` for `errorViolations`.
Check the NiFi log when a member you expected does not appear in the response.

NOTE: These six keys are reserved. Unlike free-form top-level cache keys they are no longer echoed back as additional response fields — they feed the `error` object instead.
...
IMPORTANT: A `404` therefore does not mean the request was never accepted — it may mean the request completed and its entry has since been evicted. Consumers must not treat `404` as proof that a submission failed. Size the cache so that eviction cannot occur inside the window in which consumers are expected to poll.

NOTE: The same `404` appears after a NiFi restart when the `DistributedMapCacheServer` has no "Persistence Directory" configured, because the cache is then held in memory only and starts empty.
