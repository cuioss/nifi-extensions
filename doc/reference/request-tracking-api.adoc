...
While in this status, `POST /attachments/{traceId}` uploads are accepted. Once the Wait/Notify signal is reached or the timeout expires, the status transitions to `PROCESSING` and further uploads are rejected with `409 Conflict`.
