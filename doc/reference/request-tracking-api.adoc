...
When a route has `tracking-enabled=true`, POST/PUT/PATCH requests return `202 Accepted` with tracking information.
...
[source,json]
----
{"traceId":"${rest.trace.id}","status":"PROCESSING","acceptedAt":"${rest.trace.accepted.at:ifElse('2026-01-01T00:00:00Z')}","updatedAt":"${now():format('yyyy-MM-dd''T''HH:mm:ss''Z''','UTC')}"}
----
...
|`restapi.<name>.tracking-enabled`
|`false`
|Enable async request tracking for this route (POST/PUT/PATCH only)
|===
