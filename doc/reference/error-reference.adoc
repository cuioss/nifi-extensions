...
|409
|Conflict
|Attachment limit reached or attachment window closed. Returned by the `/attachments` endpoint when the route's `attachments-max-count` is exceeded or the parent request has already transitioned past `COLLECTING_ATTACHMENTS`.
