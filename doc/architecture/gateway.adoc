...
5. **Body Validation** -- Reads the request body up to the configured maximum size. Returns 413 if the body exceeds the limit. If a route specifies a JSON Schema (`schemaName`), returns 422 on validation failure.
...
|de.cuioss.cui-http:cui-http |HTTP security pipeline (input sanitization, security event counters)
