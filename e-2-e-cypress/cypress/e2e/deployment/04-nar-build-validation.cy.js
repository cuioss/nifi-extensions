 ...
 * 
 * @fileoverview NAR build and deployment validation for NiFi JWT extension
...
describe('04 - NAR Package and Build Validation', () => {
  ...
  context('NAR File Structure Validation', () => {
    ...
    it('R-BUILD-001: Should validate NAR file exists and is accessible', () => {
      ...
      cy.request({
        ...
        if (response.status === 200) {
          ...
          const customProcessors = processorTypes.filter(processor => 
            processor.type.includes('cuioss') || 
            processor.type.includes('JWT') ||
            processor.type.includes('JWKS')
          );
          
          logTestStep('04-nar-validation', `Found ${customProcessors.length} custom processors in API`);
          expect(customProcessors.length).to.be.at.least(0); // May be 0 if not deployed yet
        } else {
          logTestStep('04-nar-validation', 'Processor types API not accessible - may require authentication');
        }
      });
    });
    ...
    it('R-BUILD-002: Should validate build artifact integrity', () => {
      ...
      validateVersionCompatibility()
        .then((compatibility) => {
          ...
          logTestStep('04-nar-validation', `Compatible with NiFi version: ${compatibility.nifiVersion}`);
    ...
    it('R-BUILD-002: Should validate required dependencies are present', () => {
      ...
      // Check that essential NiFi dependencies are available
      ...
      cy.window().then(() => {
        ...
        cy.get('body').then(($body) => {
          ...
          if (hasNiFiUI && !hasError) {
            ...
            logTestStep('04-nar-validation', 'Error detected in NiFi UI - possible dependency issues');
          } else {
            logTestStep('04-nar-validation', 'NiFi UI not fully loaded - dependency status unclear');
          }
        });
      });
    });
    ...
    it('R-INTEGRATION-001: Should validate no classloader conflicts', () => {
      ...
      cy.visit('/nifi')
        ...
        .then(($body) => {
          ...
          const hasClassloaderErrors = $body.find('.error').filter((index, element) => {
            const text = element.textContent.toLowerCase();
            return text.includes('classloader') || 
          ...
          const hasWorkingUI = $body.find('#canvas-container, .canvas').length > 0 ||
          ...
          cy.request({
            ...
            if (response.status === 200) {
              ...
              if (Array.isArray(processorTypes)) {
                const standardProcessors = processorTypes.filter(processor => 
                  processor.type && processor.type.includes('org.apache.nifi')
                );
                ...
                logTestStep('04-nar-validation', `Found ${standardProcessors.length} standard processors`);
                logTestStep('04-nar-validation', `Found ${customProcessors.length} custom processors`);
                
                // Both should coexist
                expect(standardProcessors.length).to.be.at.least(1);
                
                logTestStep('04-nar-validation', 'Extension coexists properly with standard processors');
              } else {
                logTestStep('04-nar-validation', 'Processor types response format unexpected');
              }
            } else {
              logTestStep('04-nar-validation', 'Processor types API not accessible - authentication may be required');
            }
          });
        })
        .catch((error) => {
    ...
    it('R-BUILD-003: Should validate build produces expected artifacts', () => {
      ...
      cy.visit('/nifi')
        ...
        .then(($body) => {
          ...
          logTestStep('04-nar-validation', 'Application appears to be built and deployed correctly');
        })
        .catch((error) => {
          ...
          cy.window().then((win) => {
            ...
            cy.wait(3000).then(() => {
              if (buildRelatedErrors.length > 0) {
                logTestStep('04-nar-validation', `Build-related errors detected: ${buildRelatedErrors.length}`);
        ...
        .then(() => {
          ...
          cy.window().then((win) => {
            ...
            if (performance && performance.getEntriesByType) {
              ...
              logTestStep('04-nar-validation', 'Performance API not available - cannot validate resource bundling');
            }
          });
        })
        .catch((error) => {
