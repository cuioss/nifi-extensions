      ...
      validateProcessorServices()
        .then((serviceValidation) => {
          ...
          expectedProcessorNames.forEach(processorName => {
            const found = serviceValidation.registeredProcessors.some(processor => 
    ...
    it('R-DEPLOY-003: Should verify processors appear in catalog', () => {
      ...
      navigateToPage('/nifi')
        ...
        .then((catalog) => {
          ...
          logTestStep('03-deployment', `Found ${jwtCount} JWT processors and ${jwksCount} JWKS processors in catalog`);
      ...
      navigateToPage('/nifi')
        ...
        .then(() => {
          ...
          return cy.get('body').then(($body) => {
            const hasJWTValidatorUI = $body.find('[data-processor-type*="JWT"], [class*="jwt-validator"]').length > 0;
            const hasJWKSUI = $body.find('[data-processor-type*="JWKS"], [class*="jwks"]').length > 0;
