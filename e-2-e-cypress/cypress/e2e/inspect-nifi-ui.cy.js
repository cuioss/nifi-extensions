    ...
    cy.get('body').then(($body) => {
      ...
      if (inputs.length > 0) {
        inputs.each((index, input) => {
          ...
          cy.log(`Input ${index}: type=${input.type}, placeholder="${$input.attr('placeholder')}", name="${$input.attr('name')}", id="${$input.attr('id')}"`);
        });
      }
      ...
      if (buttons.length > 0) {
        buttons.each((index, button) => {
          ...
          cy.log(`Button ${index}: text="${text}", type="${$btn.attr('type')}", class="${$btn.attr('class')}"`);
        });
      }
      ...
      const canvasElements = $body.find('*').filter((i, el) => {
        ...
        return id.includes('canvas') || className.includes('canvas') || 
               id.includes('toolbar') || className.includes('toolbar') ||
               id.includes('flow') || className.includes('flow');
      });
