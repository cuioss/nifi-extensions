/**
 * @fileoverview Multi-Issuer JWT Validation test implementation for NiFi JWT extension
 ...
 * 
 * @requirements R-JWT-001, R-JWT-002, R-JWT-003, R-JWT-004, R-JWT-005
...
function configureMultipleIssuers(processorId) {
  TEST_ISSUERS.forEach((issuer, index) => {
    cy.wrap(configureProcessorProperty(processorId, `issuer.${index + 1}.name`, issuer.name))
      .then(() => {
        logTestStep('08-jwt-multi', `Configured issuer ${index + 1}: ${issuer.name}`);
      });

    cy.wrap(configureProcessorProperty(processorId, `issuer.${index + 1}.jwks-uri`, issuer.jwksUrl))
      .then(() => {
        logTestStep('08-jwt-multi', `Configured JWKS URL for issuer ${index + 1}`);
      });

    cy.wrap(configureProcessorProperty(processorId, `issuer.${index + 1}.algorithm`, issuer.algorithm))
      .then(() => {
        logTestStep('08-jwt-multi', `Configured algorithm ${issuer.algorithm} for issuer ${index + 1}`);
      });

    cy.wrap(configureProcessorProperty(processorId, `issuer.${index + 1}.audience`, issuer.audience))
      .then(() => {
  ...
  invalidConfigs.forEach((config) => {
    cy.wrap(configureProcessorProperty(processorId, config.property, config.value))
      .then((result) => {
        ...
        logTestStep('08-jwt-multi', `Invalid config rejected: ${config.property} = ${config.value}`);
      });
  });
}
  ...
  context('Multi-Issuer JWT Token Processing', () => {
    ...
    it('R-JWT-004: Should support multiple JWT algorithms', () => {
      ...
      algorithmTests.forEach((algorithm) => {
        cy.wrap(testAlgorithmValidation(algorithm))
          .then(() => {
    ...
    it('R-JWT-006: Should meet performance requirements for JWT validation', () => {
      ...
      cy.wrap(validatePerformanceMetrics(performanceTests, {
        ...
        .then((metrics) => {
          ...
          logTestStep('08-jwt-multi', `Performance validated: avg=${metrics.averageResponseTime}ms, max=${metrics.maxResponseTime}ms, success=${metrics.successRate * 100}%`);
        })
        .catch((error) => {
  ...
  context('Error Handling and Edge Cases', () => {
    ...
    it('R-JWT-009: Should handle malformed configuration gracefully', () => {
      ...
      const invalidConfigs = [
        ...
        { property: 'issuer.1.algorithm', value: 'INVALID_ALG', expectedError: 'unsupported algorithm' },
