...
describe('NiFi Advanced Settings Test', () => {
  ...
  it('should access MultiIssuerJWTTokenAuthenticator advanced settings', () => {
    ...
    cy.get('body').then(($body) => {
      ...
      cy.log(
        `Canvas environment: Add button: ${hasAddButton}, Toolbar: ${hasToolbar}, Canvas: ${hasCanvas}`
      );
      ...
      if (hasAddButton) {
        ...
        cy.get('body').then(($bodyAfterAdd) => {
          ...
          const hasProcessorList =
            $bodyAfterAdd.find('*').filter((i, el) => {
              return (
                Cypress.$(el).text().includes('MultiIssuer') || Cypress.$(el).text().includes('JWT')
              );
            }).length > 0;

          cy.log(
            `After add click: Dialog: ${hasDialog}, JWT processor visible: ${hasProcessorList}`
          );
          ...
          if (hasProcessorList) {
            ...
            cy.get('body').then(($bodyWithProcessor) => {
              const hasConfirmButton =
                $bodyWithProcessor.find('button').filter((i, btn) => {
        ...
        cy.log(
          'No add processor button found, testing configuration access with existing elements'
        );
        cy.testProcessorAdvancedSettings();
      }
    });
  ...
  it('should access token verification interface', () => {
    ...
    cy.get('body').then(($body) => {
      const hasTokenElements =
        $body.find('*').filter((i, el) => {
          const text = Cypress.$(el).text().toLowerCase();
          const className = Cypress.$(el).attr('class') || '';
          return (
            (text.includes('token') && text.includes('verify')) ||
  ...
  it('should validate JWT tokens with processor', () => {
    ...
    cy.generateTestToken({ sub: 'test-user', scope: 'read write' }).then((validToken) => {
      ...
      cy.get('body').then(($body) => {
        const hasProcessors =
          $body.find('g.processor, [class*="processor"], .component').length > 0;
  ...
  it('should handle missing required processor properties', () => {
    ...
    cy.get('body').then(($body) => {
      const hasAddButton =
        $body.find('button, [class*="add"], [class*="toolbar"]').filter((i, el) => {
          const text = Cypress.$(el).text().toLowerCase();
      ...
      if (hasAddButton) {
        ...
        cy.get('button, [class*="add"], [class*="toolbar"]')
          .contains(/add|processor/i)
          .first()
          .click({ force: true });
        cy.wait(1000);
        ...
        cy.get('body').then(($bodyWithDialog) => {
          const hasProcessorList =
            $bodyWithDialog.find('*').filter((i, el) => {
