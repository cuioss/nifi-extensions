...
describe('NiFi UI Analysis for Processor Addition', () => {
  ...
  it('should analyze UI structure for processor addition patterns', () => {
    ...
    cy.get('body').then(($body) => {
      ...
      // Analyze toolbars
      $body.find('[role="toolbar"], .toolbar, .mat-toolbar, .flow-toolbar').each((index, element) => {
        const $toolbar = Cypress.$(element);
        const buttons = $toolbar.find('button').length;
        analysis.toolbars.push({
          classes: $toolbar.attr('class') || '',
          buttonCount: buttons,
          hasAddButton: $toolbar.find('button').toArray().some(btn => 
      ...
      if (analysis.addButtons.length > 0) {
        ...
        analysis.addButtons.forEach((btn, index) => {
          cy.log(`  ${index + 1}. Text: "${btn.text}", Title: "${btn.title}", Aria: "${btn.ariaLabel}"`);
        });
      }
      ...
      if (analysis.canvas.length > 0) {
        ...
        analysis.canvas.forEach((canvas, index) => {
          cy.log(`  ${index + 1}. ${canvas.tagName}, Classes: "${canvas.classes}", Size: ${canvas.width}x${canvas.height}`);
        });
      }
          ...
          cy.get('body').then(($checkBody) => {
            ...
            if (dialogs.length > 0) {
              ...
              if (dialogText.includes('processor') || dialogText.includes('component')) {
                ...
                cy.get('button').contains(/cancel|close|×/i).first().click({ force: true });
              } else {
                // Close other dialogs
                cy.get('button').contains(/cancel|close|×/i).first().click({ force: true });
              }
        ...
        if (elements.length > 0) {
          ...
          cy.get('body').then(($contextBody) => {
            ...
            if (contextMenus.length > 0) {
              ...
              if (menuText.includes('processor') || menuText.includes('add') || menuText.includes('component')) {
