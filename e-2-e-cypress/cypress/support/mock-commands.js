 ...
 * Updated to use Angular Material framework patterns discovered in Phase 0
 * @version 1.1.0
 */
...
Cypress.Commands.add('setupMockedNiFi', (options = {}) => {
  logMessage('info', 'Setting up mocked Angular Material NiFi environment');

  // Set up API mocks
  setupNiFiAPIMocks(options);

  // Mock the Angular Material canvas structure in DOM
  cy.get('body').then(($body) => {
    // Create mock Angular Material structure if it doesn't exist
    if ($body.find('mat-sidenav-content').length === 0 && $body.find('router-outlet').length === 0) {
      $body.append(`
  ...
  logMessage('success', 'Mocked Angular Material NiFi environment ready');
});
...
Cypress.Commands.add('findMockedProcessorOnCanvas', (processorType, options = {}) => {
  ...
  // Use Angular Material canvas selectors
  return cy.get(SELECTORS.CANVAS_CONTAINER).then(($canvasContainer) => {
    const processorSelector = `[data-processor-type="${processorType}"]`;
    const $processor = $canvasContainer.find(processorSelector);
...
function performMockedProcessorAddition(processorType, processorDef, position) {
  ...
  // Add visual element to mocked Angular Material canvas
  return cy.get(`${SELECTORS.CANVAS_SVG} .processors-layer`).then(($layer) => {
...
function performMockedProcessorRemoval(processor, confirmDeletion) {
  ...
  // Remove visual element from mocked Angular Material canvas
  return cy.get(SELECTORS.CANVAS_CONTAINER).then(() => {
