...
Cypress.Commands.add('testFirefoxQuirks', (win) => {
  ...
  cy.log('Testing Firefox-specific behaviors');
});
...
Cypress.Commands.add('testWebKitQuirks', (win) => {
  ...
  cy.log('Testing WebKit-specific behaviors');
});
...
Cypress.Commands.add('testFetchAPIConsistency', () => {
  cy.window().then((win) => {
    if (typeof win.fetch !== 'undefined') {
      ...
      return win.fetch('data:text/plain,test')
...
Cypress.Commands.add('verifyDOMManipulationConsistency', () => {
  ...
  cy.get('.property-input').first().then(($input) => {
    ...
    cy.wrap($input).clear().type(initialValue || '');
  });
});
...
Cypress.Commands.add('measureRenderingPerformance', () => {
  return cy.window().then((win) => {
    ...
    cy.get('.configuration-dialog').should('be.visible').then(() => {
    ...
    cy.get('.property-list, .properties-table').should('be.visible').then(() => {
