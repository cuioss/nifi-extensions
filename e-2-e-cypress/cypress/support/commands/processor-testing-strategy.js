  ...
  return cy.get('body').then(($body) => {
    ...
    if (processorCount > 0) {
      ...
      if (skipIfNone) {
        ...
        throw new Error('No processors available for testing. Please add processors to the NiFi canvas manually.');
      }
    }
  });
});
...
Cypress.Commands.add('testFirstAvailableProcessor', () => {
  ...
  return cy.get(SELECTORS.PROCESSOR).first().then(($processor) => {
    ...
    cy.get('body').then(($body) => {
      ...
      if (contextMenus.length > 0) {
        ...
        const configOptions = contextMenus.filter(':contains("Configure"), :contains("Properties"), :contains("Edit")');
...
Cypress.Commands.add('testProcessorType', (processorType) => {
  ...
  return cy.get('body').then(($body) => {
    ...
    const matchingProcessors = $body.find(SELECTORS.PROCESSOR).filter((index, element) => {
      ...
      return text.includes(processorType.toLowerCase()) || 
...
Cypress.Commands.add('testProcessorElement', ($processor, expectedType = 'unknown') => {
  ...
  cy.wrap($processor).then(($el) => {
    ...
    const hasValidState = classes.includes('valid') || classes.includes('stopped') || classes.includes('running');
  ...
  return cy.get('body').then(($body) => {
    ...
    if (contextMenus.length > 0) {
      ...
      if (configText.includes('configure') || configText.includes('properties') || configText.includes('edit')) {
...
export {
  // Export functions for potential direct use
};
