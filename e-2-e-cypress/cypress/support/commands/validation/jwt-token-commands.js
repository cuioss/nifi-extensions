...
Cypress.Commands.add('verifyTokenValidation', (processorId, token) => {
  ...
  cy.get('body').then(($body) => {
    ...
    if (hasTokenInput && hasVerifyButton) {
      // Input the token
      cy.get('textarea, input[placeholder*="token"], .token-input')
        .first()
        .clear()
        .type(token, { delay: 0 });
      ...
      cy.get('body').then(($resultBody) => {
        const hasResults =
          $resultBody.find('.results, .token-results, .verification-result').length > 0;
        ...
        cy.log(
          `Token validation results: Results area: ${hasResults}, Success: ${hasSuccess}, Error: ${hasError}`
        );
