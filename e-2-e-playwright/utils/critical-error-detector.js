...
/**
 * Check if an error message matches a whitelisted pattern
 * @param {string} message - error message to check
 * @returns {boolean} true if the message is whitelisted
 */
export function isErrorWhitelisted(message) {
    return ERROR_WHITELIST.some((pattern) => message.includes(pattern));
}

/**
 * Check loading indicator status on the page
 * @param {import('@playwright/test').Page} page - Playwright page
 * @returns {Promise<{isVisible: boolean, selector: string|null, text: string|null}>} loading status
 */
export async function checkLoadingIndicatorStatus(page) {
    ...
    return { isVisible: false, selector: null, text: null };
}
...
const CRITICAL_ERROR_PATTERNS = {
    ...
    PAGE_ERRORS: [/Page crashed/i, /Navigation failed/i, /Script timeout/i],
};
...
export class CriticalErrorDetector {
    ...
    checkConsoleMessage(msg, testInfo) {
        ...
        const isSuccessMessage =
            /Loading indicator hidden|successfully hidden|initialization completed|Component registration successful/i.test(
                message,
            );
        ...
        // Check for JavaScript errors (skip whitelisted messages)
        if (type === "error" && !isErrorWhitelisted(message)) {
    ...
    checkPageError(error, testInfo) {
        ...
        if (isErrorWhitelisted(message)) return;

        // Non-whitelisted page errors are critical
        this.addCriticalError(
            "PAGE_ERROR",
            `Page Error: ${message}`,
            testInfo,
...
/**
 * Convenience function to clear and start error monitoring for a test.
 * Used by the page fixture as a one-liner setup.
 * @param {import('@playwright/test').Page} page - Playwright page
 * @param {import('@playwright/test').TestInfo} testInfo - Playwright testInfo
 */
export function setupErrorMonitoring(page, testInfo) {
