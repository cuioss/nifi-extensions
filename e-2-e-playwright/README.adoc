...
│   ├── 01..07-*.spec.js           # Functional tests (processor features)
...
├── utils/                        # Utility services and scripts (12 files)
...
=== Functional Tests (`01-` through `07-*.spec.js`)

Tests for specific JWT processor features: custom UI tabs, JWKS validation, token verification, metrics, help content, gateway tabs, and context help.
...
2. *Keycloak* accessible at `PLAYWRIGHT_KEYCLOAK_URL` (default: `https://localhost:9085`)
...
Once running, NiFi is available at `https://localhost:9095/nifi` and Keycloak at `https://localhost:9085`.
...
[source,bash]
----
# Full integration test suite: starts containers → runs all tests → stops containers
./mvnw clean verify -pl e-2-e-playwright -Pintegration-tests -am
----

Test results are stored under `target/test-results/`. Reports are in `target/test-results.json` (JSON) and `target/junit-results.xml` (JUnit).
...
* *Artifacts*: Trace `retain-on-failure`, screenshots `only-on-failure`, video `retain-on-failure`
* *Parallel*: `fullyParallel: false`, 1 worker (sequential execution)
...
|HTML report (only generated when using `--reporter=html`, e.g. `npm run test:a11y:full`)
|===
...
|`CI`
|CI environment flag (affects `forbidOnly`)
|(unset)
|===
...
* xref:docs/test-logging.adoc[Test Logging Architecture] — Per-test artifact persistence and log formats
