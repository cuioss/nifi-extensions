/**
 * @fileoverview Self-test for Keycloak Token Service
 * Validates that the token service can fetch valid tokens and handle errors appropriately
 */

import { test, expect } from '@playwright/test';
import { KeycloakTokenService, getValidAccessToken, getInvalidAccessToken } from '../utils/keycloak-token-service.js';
...
test.describe('Keycloak Token Service', () => {
  ...
  test('should fail loudly when Keycloak is not accessible', async () => {
    ...
    await expect(badService.getValidAccessToken()).rejects.toThrow(/CRITICAL: Failed to obtain access token from Keycloak/);
    await expect(badService.getValidAccessToken()).rejects.toThrow(/run-and-deploy.sh/);
  });
  ...
  test('should fail loudly with invalid credentials', async () => {
    ...
    await expect(customService.getValidAccessToken()).rejects.toThrow(/Failed to obtain access token from Keycloak/);
  });
  ...
  test('should expose token endpoint for debugging', () => {
    ...
    expect(endpoint).toContain('/realms/oauth_integration_tests/protocol/openid-connect/token');
