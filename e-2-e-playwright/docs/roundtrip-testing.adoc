...
**Console Logs** (all tests follow this pattern):
[source]
----
...
**Test-specific Summary Reports** (when applicable):
[source]
----
[test-file]-[test-name]-summary-YYYY-MM-DDTHH-MM-SS-sssZ.json
----
...
# List summary reports (when available)
ls -la e-2-e-playwright/target/logs/*summary*.json | tail -5
----
...
3. **Review Test Summary Reports** (when available):
+
[source,bash]
----
# View test summary report
cat e-2-e-playwright/target/logs/*summary*.json | jq '.'
----
...
* **Loading Indicator Status**: Check `loadingIndicatorVisible` in test summary reports
...
**Test Summary Structure** (when available):
[source,json]
----
{
...
**Purpose**: The JWT UI console error capture test is part of the unified logging system and automatically collects and analyzes browser console logs for JWT UI functionality.

**Test File**: `tests/capture-jwt-console.spec.js`

**Features**:
* **Unified Logging Integration**: Uses the same logging system as all other tests
...
**Output Files** (follows unified logging format):
* `capture_jwt_console_spec_js-Capture_JWT_UI_Console_Errors-console-logs-*.json` - Console logs (JSON)
* `capture_jwt_console_spec_js-Capture_JWT_UI_Console_Errors-console-logs-*.log` - Console logs (human-readable)
* `capture-jwt-console-JWT_UI_Console_Error_Capture-summary-*.json` - Test summary report
...
[NOTE]
====
The JWT UI console error capture test uses the same unified logging system as all other tests. The only difference is that it includes an additional summary report with JWT UI specific information.
====
...
# Check test summary reports (if available)
cat target/logs/*summary*.json | jq '.loadingIndicatorVisible'
