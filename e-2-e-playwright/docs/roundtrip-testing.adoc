...
Maven starts containers, runs all Playwright tests in a single invocation (auth-setup → self-tests → functional + accessibility in parallel), and stops containers afterward. Test results are stored under `target/test-results/`.
...
* `target/test-results/` — Per-test execution artifacts (browser.log, test-run.log, screenshots, video, trace.zip)
...
grep -n "ERROR" target/test-results/*/browser.log

# View screenshots from failed tests
open target/test-results/*/*.png
