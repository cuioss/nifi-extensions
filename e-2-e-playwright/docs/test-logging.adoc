...
Every test typically produces four files in its `testInfo.outputDir`:
...
// Typical pattern (most test files)
test.beforeEach(async ({ page }, testInfo) => {
    await takeStartScreenshot(page, testInfo);
});

// Extended pattern (self-tests that need processor management)
// Requires additional import: import { AuthService } from "../utils/auth-service.js";
...
[source]
----
[22:37:17.172] [info] [JWT-UI] Component UI initialising...
