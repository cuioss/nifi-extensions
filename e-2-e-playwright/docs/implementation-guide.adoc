...
│   ├── CanvasPage.js            # Canvas page interactions
│   └── index.js                 # Page fixtures + re-exports
...
└── config/                       # Configuration
    ├── accessibility-config.js  # WCAG axe-core rules
    └── console-error-config.js  # Console error patterns
----
...
Page Objects are available as Playwright fixtures:

[source,javascript]
----
import { test, expect } from '../pages/index.js';

test('Authentication and navigation', async ({ loginPage, canvasPage }) => {
...
test('Test with full instrumentation', async ({ page, accessibilityHelper }) => {
