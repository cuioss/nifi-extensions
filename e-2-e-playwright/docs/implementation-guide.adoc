...
│   ├── NN-*.spec.js             # Functional tests (01-07, numbered for execution order)
...
The `test-fixtures.js` provides extended fixtures with automatic console logging and error detection. Key exports: `test`, `accessibilityTest`, `serialTest`, `gatewayTest`, `serialGatewayTest`, `expect`, and `takeStartScreenshot`.
...
* `01..07-*.spec.js` — Functional tests (numbered for execution order)
* `accessibility-*.spec.js` — WCAG compliance tests
...
|`self-login.spec.js`, `self-critical-errors.spec.js`
...
When run via Maven (`-Pintegration-tests`), tests execute in phases:

1. Auth-setup — one-time authentication via `global-setup.js`, saves storage state
2. Self-tests — if these fail, subsequent phases are skipped (depends on auth-setup)
3. Functional tests and accessibility tests (both depend on self-tests; they are independent of each other)
...
* `--disable-features=VizDisplayCompositor` — rendering stability
* `--enable-automation` — automation support
...
* *Console logs*: Saved automatically by test fixtures as `browser.log` and `test-run.log` in each test's output directory
...
* xref:test-logging.adoc[Test Logging Architecture] — Logging infrastructure and artifact generation
