...
*Current Version*: Apache NiFi 2.6.0 (Angular Material UI)

*UI Sources*: https://github.com/apache/nifi/tree/rel/nifi-2.6.0/nifi-web/nifi-web-frontend
...
NiFi requires a CSRF token for API authentication. The token is obtained from cookies after initial page load:
...
NiFi health check (curl-based, HTTPS):
...
Keycloak health check (TCP socket-based, port 9000):

[source,yaml]
----
healthcheck:
  test: ["CMD-SHELL", "exec 3<>/dev/tcp/localhost/9000 && ... | grep -q 'UP' ..."]
  interval: 10s
  timeout: 5s
  retries: 12
  start_period: 30s
