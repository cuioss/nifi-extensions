...
Accessibility tests run via `exec-maven-plugin` in the `integration-test` phase when the `integration-tests` profile is active. They execute in parallel with functional tests after self-tests pass (both depend on `self-tests` in the Playwright config).
