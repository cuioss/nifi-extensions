...
* **axe-playwright**: Core accessibility testing engine
...
Accessibility tests run via the `frontend-maven-plugin` in the `test` phase (skipped unless the `integration-tests` profile is active):

[source,xml]
----
<execution>
  <id>accessibility-tests</id>
  <goals>
    <goal>npm</goal>
  </goals>
  <phase>test</phase>
  <configuration>
    <arguments>run test:a11y:quick</arguments>
