...
test('My test that needs the processor', async ({ page }) => {
  ...
  // Setup processor — throws if processor cannot be placed on canvas
...
test.beforeEach(async ({ page }) => {
  ...
  // Ensure processor is on canvas (returns false if setup fails)
...
In contrast, `ProcessorApiManager.ensureProcessorOnCanvas()` throws on failure and always returns `true` on success.
...
test.describe('Tests requiring MultiIssuerJWTTokenAuthenticator', () => {
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    // ensureProcessorOnCanvas() throws if processor cannot be placed
...
. *Always authenticate first*: Ensure proper authentication before using ProcessorApiManager
. *Check deployment*: Verify processor is deployed before attempting to add to canvas
. *Handle failures gracefully*: Use test.skip() when processor is not available
. *Clean up in tests*: Remove test processors after completion when appropriate
. *Use the helper functions*: Leverage processor-setup-helper.js for common patterns
