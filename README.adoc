...
Custom https://nifi.apache.org/[Apache NiFi] processors for JWT-authenticated REST API handling. The project provides two complementary processors:

* **MultiIssuerJWTTokenAuthenticator** -- validates JWT tokens from FlowFile attributes against multiple identity providers (Keycloak, Entra ID, Auth0) with automatic JWKS key rotation and caching
* **RestApiGatewayProcessor** -- a self-contained REST API gateway with an embedded Jetty HTTP server, route-based dispatching, per-route JWT authorization, and RFC 9457 error responses

Both processors share JWT validation through a common **JwtIssuerConfigService** Controller Service.

== The Problem

NiFi flows that consume REST APIs or process webhook payloads often receive JWT-authenticated requests. Out of the box, NiFi has no processor to validate these tokens. Building a REST API gateway manually requires chaining `HandleHttpRequest` + custom validation + `HandleHttpResponse` with complex error routing.

== The Solution

**For FlowFile-based JWT validation**: The `MultiIssuerJWTTokenAuthenticator` drops into any NiFi flow as a single node. It reads the JWT from a FlowFile attribute, validates it via a shared Controller Service, and writes the token claims as FlowFile attributes.

image::doc/plantuml/jwt-token-flow.png[JWT Token Processing Flow]

**For REST API endpoints**: The `RestApiGatewayProcessor` is a single processor that embeds an HTTP server, authenticates and authorizes requests per-route, and converts them into FlowFiles -- replacing multi-processor gateway patterns.
...
. The processors **MultiIssuerJWTTokenAuthenticator** and **RestApiGatewayProcessor** appear in the "Add Processor" dialog under the tags `jwt`, `oauth`, `authentication`.
...
=== Working Example: RestApiGateway

The integration tests also include a `RestApiGatewayProcessor` configured on port 9443 with JWT authentication via Keycloak. The gateway demonstrates:

* Multi-route configuration via dynamic properties
* Per-route JWT authorization with required roles
* RFC 9457 error responses for authentication/authorization failures
* `/metrics` management endpoint with Prometheus-compatible output
...
* link:doc/rest-errors/index.adoc[REST API Error Reference (RFC 9457)]
