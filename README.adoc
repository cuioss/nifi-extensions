...
image:https://img.shields.io/badge/Apache%20NiFi-2.10.0-1f6aa5[Apache NiFi,link=https://nifi.apache.org/]
...
image::doc/resources/diagrams/api-gateway-pattern.svg[RestApiGateway Pattern]
...
image::doc/resources/diagrams/tracking-modes.svg[Request Tracking Modes]

* **Simple** (`tracking-mode=simple`): `202` with `traceId` and `/status` polling link
* **Attachments** (`tracking-mode=attachments`): adds an `/attachments/{traceId}` upload endpoint; the parent request waits for attachments (or timeout), then the window closes (`409`). See link:doc/guides/AttachmentMergeGuide.adoc[Attachment Merge Guide].
...
* link:doc/architecture/jwt-processor.adoc[JWT Processor Architecture]: flow, attributes, Token-Sheriff lifecycle
...
* link:doc/guides/AttachmentMergeGuide.adoc[Attachment Merge Guide]: Wait/Notify synchronization for parent requests with attachments
...
* link:doc/reference/request-tracking-api.adoc[Request Tracking API]: async request tracking with traceId and status polling
* link:doc/reference/request-tracking-architecture.adoc[Request Tracking Architecture]: status lifecycle, data model, sequence diagrams
* link:doc/reference/attachments-api.adoc[Attachments API]: chained attachment uploads with limit enforcement
* link:doc/reference/metrics-api.adoc[Metrics API]: gateway `/metrics` endpoint and NiFi-native counter exposition

=== Migration

* link:doc/migration/token-sheriff-0.9.0.adoc[OAuth-Sheriff → Token-Sheriff (0.9.0)]: library rename, config-key and Java API migration notes
