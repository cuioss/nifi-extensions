...
=== Quick Start: Minimal Configuration via UI

. Drag the **MultiIssuerJWTTokenAuthenticator** onto the canvas.
. In the processor configuration dialog, the defaults work for the common case (Authorization header, Bearer prefix).
. Add a dynamic property for your identity provider:
+
[cols="1,2"]
|===
|Property Name |Value
...
|`issuer.1.audience` _(optional)_
|`api://your-service`
|===
. Connect the three relationships: `success`, `authentication-failed`, `failure`.
. Start the processor.
