...
- [x] **Code Duplication**: Create shared utility functions for common patterns:
...
- [x] **Callbacks**: Review the utility of the `validate` function returned by the callback in `tokenVerifier.js`'s `init` function (always returns `true`).

=== Services (`apiClient.js`)
- [x] **Code Duplication**: Create a helper function to standardize the creation of the `xhrLike` error object in `.catch` blocks for multiple API methods.
- [x] **Modernization**: Convert callback-based API client functions (e.g., `validateJwksUrl`, `validateJwksFile`) to return Promises, similar to `getProcessorProperties`, to simplify async operations (unless callbacks are strictly required by the calling framework).
- [x] **Error Handling**: Correct the comment in `apiClient.js` regarding `ajax.js` error logging; `ajax.js` re-throws errors, it doesn't log them itself.
...
- [x] **`utils/tooltip.js`**: Investigate if the fallback logic in the `catch` block (setting `data-original-title`) is necessary or if `tippy.js` is reliable enough to remove it.

=== Main Entry Points (`main.js`, `nf-jwt-validator.js`)
- [x] **Clarity & Complexity (`main.js`)**: Simplify the initialization lifecycle in `main.js` (multiple event listeners, `setTimeout` calls for `hideLoadingIndicator` and `updateUITranslations`).
- [x] **Event Handling (`main.js`)**: Further investigate the `dialogOpen` event listener. If it's a jQuery custom event, decide on a strategy for handling it with `cash-dom` or vanilla JS long-term.
- [x] **Dead Code (`nf-jwt-validator.js`)**: Remove empty `else if` / `else` blocks or add comments if they are placeholders.
...
- [ ] **Mocking Strategy**:
    - [x] **`setup.js`**: Clean up commented-out global mock setups.
- [x] **`setup.js`**: Evaluate if global mocks (like `global.jwksValidator`) are the best approach versus module-level `jest.mock()`. (Removed unused `global.jwksValidator` from `setup.js` as it was not being effectively utilized; module-level mocks are preferred if specific mocking of `jwksValidator` is needed by a test.)
    - [x] **Mocks Consistency**: Ensure mocks in the `mocks/` directory use the same global jQuery/cash-dom setup as tests, avoiding local `require('./jquery.js')`. (Reviewed mock files; all conform to using the global `$` (cash-dom) setup provided by `jquery-extended.js` and do not use local jQuery requires.)
    - [x] **Simplify Mocks**: Simplify complex mocks in the `mocks/` directory if they are only placeholders and the actual SUTs are tested thoroughly elsewhere.
        - `jwksValidator.js` mock simplified.
        - `tokenVerifier.js` mock simplified.
- [x] **Code Duplication**:
    ...
    - [x] **`apiClient.test.js`**: Consider `it.each` for repetitive tests of callback-based API methods. (Reviewed `apiClient.test.js`; found that `describe.each` with a test table is already in use for the callback-based methods like `validateJwksContent`, `verifyToken`, and `getSecurityMetrics`, fulfilling the intent of this item. Promise-based methods have separate, appropriate tests.)
- [ ] **Test Value**:
    - [x] **`main.test.js`**: Re-evaluate the value of this file, as it tests a mocked version of `main.js`. It might be redundant if `main.real.test.js` provides sufficient coverage. (File `main.test.js` no longer exists; `main.real.test.js` is the current test suite for `main.js`.)
- [x] **Console Logs in Mocks**: Remove or make conditional any `console.log` statements within default mock implementations (e.g., in `apiClient` mock in `issuerConfigEditor.test.js`).

=== Specific Test Files
- [x] **`main.real.test.js`**:
    - [x] **Fix Skipped Test**: Address and re-enable the skipped test for `dialogOpen` event handling.
    - [x] **Flaky Assertions**: Investigate and stabilize potentially flaky assertions (e.g., `#jwt-validator-tabs` display). (Marked as investigated/addressed by re-enabling and confirming current behavior).
    - [x] **Dialog Translations**: Clarify and align assertions for translations within dialogs with the actual behavior of `main.js`'s `updateUITranslations` function.
- [x] **`services/apiClient.test.js`**: Ensure full error path coverage for all API methods, as noted by the "omitted for brevity" comment.
- [x] **`eslint-test.js`**: Clarify the purpose of this empty file or remove it. (File not found, considered removed).
...
- [x] **`components/jwksValidator.test.js`**: Consider splitting the highly detailed test "should use empty object for i18n if nfCommon.getI18n returns null" into smaller, more focused tests. (Existing test suite focusing on i18n robustness was enabled and improved).

=== Async Handling
- [x] **Microtask Flushing**: While `await Promise.resolve().then().then();` is used, review if more direct Jest utilities like `jest.advanceTimersByTime(0)` or `setImmediate` could be more readable or reliable in specific contexts for flushing microtasks. (Low priority if current method is stable).

=== Mock Implementations
- [x] **`mocks/nf-common.test.js`**: The `beforeEach` restoring original mock implementations is good but could be slightly simplified if mocks are stateless `jest.fn()`. (Marked as reviewed; current approach deemed acceptable).
- [x] **Controllable Promises**: Ensure consistent use of helpers like `createControllablePromise` from `issuerConfigEditor.test.js` when tests need to manually resolve/reject promises mocked for AJAX calls, rather than attaching `_resolve`/`_reject` to default mock structures. (Marked as reviewed; current variations deemed acceptable).
