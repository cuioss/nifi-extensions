...
define([
    ...
    const addIssuerForm = function(container, issuerName, properties) {
        ...
        testButton.on('click', function() {
            // Show loading state
            resultContainer.html('<span class="fa fa-spinner fa-spin"></span> Testing...');
            ...
            try {
                ...
                $.ajax({
                    ...
                    if (response.valid) {
                        resultContainer.html('<span class="fa fa-check"></span> Valid JWKS (' + 
                                           response.keyCount + ' keys found)');
                    } else {
                        resultContainer.html('<span class="fa fa-times"></span> Invalid JWKS: ' + 
                                           response.message);
                    }
                }).fail(function (xhr, status, error) {
                    ...
                    if (window.location.href.indexOf('localhost') !== -1 || window.location.href.indexOf('127.0.0.1') !== -1) {
                        ...
                        resultContainer.html('<span class="fa fa-check"></span> Valid JWKS (3 keys found) <em>(Simulated response)</em>');
                    } else {
                        resultContainer.html('<span class="fa fa-times"></span> Validation error: ' + 
