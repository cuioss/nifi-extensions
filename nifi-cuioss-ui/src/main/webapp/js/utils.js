...
export const TRANSLATIONS = {
    ...
    de: {
        ...
        'route.form.connection.placeholder': 'Verbindungsbezeichnung auf der NiFi-Leinwand',
        ...
        'contexthelp.global.max.request.size': 'Maximale Anfragek\u00f6rpergr\u00f6\u00dfe in Bytes. Anfragen, die dieses Limit \u00fcberschreiten, werden mit HTTP 413 abgelehnt.',
        'contexthelp.global.queue.size': 'Maximale Anzahl wartender Anfragen, bevor der Server neue Verbindungen ablehnt.',
        ...
        'contexthelp.global.cors.origins': 'Kommagetrennte Liste erlaubter CORS-Urspr\u00fcnge. Verwenden Sie * f\u00fcr alle Urspr\u00fcnge (nicht empfohlen f\u00fcr Produktion).',
        'contexthelp.global.listening.host': 'Netzwerkschnittstelle f\u00fcr die Server-Bindung. 0.0.0.0 f\u00fcr alle Schnittstellen oder 127.0.0.1 nur f\u00fcr Loopback.',
        'contexthelp.route.name': 'Eindeutiger Bezeichner f\u00fcr diese Route. Wird als Pr\u00e4fix in NiFi-Eigenschaften verwendet (restapi.<name>.*).',
        'contexthelp.route.enabled': 'Ob diese Route aktiv ist. Deaktivierte Routen werden beim Request-Matching ignoriert.',
        'contexthelp.route.path': 'URL-Pfadmuster, das diese Route abgleicht. Eingehende Anfragen mit diesem Pfadpr\u00e4fix werden hierher geleitet.',
        'contexthelp.route.methods': 'Kommagetrennte HTTP-Methoden, die von dieser Route akzeptiert werden (z.B. GET,POST). Leer bedeutet alle Methoden.',
        'contexthelp.route.roles': 'Kommagetrennte Rollen, die im JWT-Token erforderlich sind. Die Anfrage wird abgelehnt, wenn dem Token eine aufgef\u00fchrte Rolle fehlt.',
        'contexthelp.route.scopes': 'Kommagetrennte Scopes, die im JWT-Token erforderlich sind. Die Anfrage wird abgelehnt, wenn dem Token ein aufgef\u00fchrter Scope fehlt.',
        'contexthelp.route.create.flowfile': 'Ob ein NiFi-FlowFile f\u00fcr \u00fcbereinstimmende Anfragen erstellt wird. Wenn deaktiviert, antwortet die Route direkt ohne Weiterleitung.',
        'contexthelp.route.connection': 'NiFi-Beziehungsname f\u00fcr die Weiterleitung von FlowFiles an die korrekte nachgelagerte Verbindung.',
        'contexthelp.route.schema': 'JSON-Schema f\u00fcr Anfragek\u00f6rper-Validierung. Anfragen, die die Schema-Validierung nicht bestehen, werden mit HTTP 400 abgelehnt.',
        'contexthelp.issuer.name': 'Eindeutiger Bezeichner f\u00fcr diesen JWT-Aussteller. Wird als Pr\u00e4fix in NiFi-Eigenschaften verwendet (issuer.<name>.*).',
        'contexthelp.issuer.jwks.type': 'Wie JWKS-Signaturschl\u00fcssel geladen werden: von einer Remote-URL, einer lokalen Datei oder als Inline-Inhalt.',
        'contexthelp.issuer.uri': 'Erwarteter Wert des "iss"-Claims in JWT-Token. Token mit einem anderen Aussteller werden abgelehnt.',
        'contexthelp.issuer.jwks.url': 'Remote-URL, die das JWKS-JSON-Dokument bereitstellt. Schl\u00fcssel werden zwischengespeichert und periodisch aktualisiert.',
        'contexthelp.issuer.jwks.file': 'Absoluter Dateipfad zu einer lokalen JWKS-JSON-Datei. N\u00fctzlich f\u00fcr isolierte Umgebungen.',
        'contexthelp.issuer.jwks.content': 'Inline-JWKS-JSON-Inhalt direkt eingef\u00fcgt. N\u00fctzlich f\u00fcr Tests oder Einzelschl\u00fcssel-Setups.',
        'contexthelp.issuer.audience': 'Erwarteter "aud"-Claim-Wert. Wenn gesetzt, werden Token ohne passende Audience abgelehnt.',
        'contexthelp.issuer.client.id': 'OAuth2-Client-ID f\u00fcr die Token-Validierung. Wird mit dem "azp"- oder "client_id"-Claim verglichen.',
