(function (p, A){ typeof exports == 'object' && typeof module < 'u' ? A(exports, require('nf.Common'), require('tippy.js')) : typeof define == 'function' && define.amd ? define(['exports', 'nf.Common', 'tippy.js'], A) : (p = typeof globalThis < 'u' ? globalThis : p || self, A(p.nifiCuiossUI = {}, p.nfCommon, p.tippy)); })(this, function (p, A, Xe){
    ...
    `;if (e.innerHTML = E, r){ const T = e.querySelector('.close-error');T && T.addEventListener('click', () => { const h = e.querySelector('.error-message');h && (h.style.transition = 'opacity 0.3s', h.style.opacity = '0', setTimeout(() => h.remove(), 300)); }); }a && setTimeout(() => { const T = e.querySelector('.error-message');T && (T.style.transition = 'opacity 0.3s', T.style.opacity = '0', setTimeout(() => T.remove(), 300)); }, 5e3);
        }, Y = (e, t, s = {}) => {
            const { autoHide: i = !0 } = s, o = `
        <div class="success-message ${i ? 'auto-dismiss' : ''}">
            <div class="success-content">${t}</div>
        </div>
    `;e.innerHTML = o, i && setTimeout(() => { const r = e.querySelector('.success-message');r && r.remove(); }, 5e3);
        }, it = e => { switch (e){ case 'validation':return 'validation-error';case 'network':return 'network-error';case 'server':return 'server-error';default:return ''; } }, me = e => { const { title: t, message: s, confirmText: i = 'Delete', cancelText: n = 'Cancel', type: o = 'danger', onConfirm: r, onCancel: a } = e;return new Promise(c => { document.querySelectorAll('.confirmation-dialog').forEach(u => u.remove());const d = nt(t, s, i, n, o), m = document.createElement('div');m.innerHTML = d;const w = m.firstElementChild;document.body.appendChild(w), ot(w, c, r, a), requestAnimationFrame(() => { w.classList.add('show');const u = w.querySelector('.cancel-button');u && u.focus(); }); }); }, z = e => { const t = document.createElement('div');return t.textContent = e, t.innerHTML; }, nt = (e, t, s, i, n) => {
            const o = at(n), r = ct(n);return `
    ...
    `;if (e.decoded){
            ...
            `;const a = e.decoded.payload;if (r += '<div class="token-claims">', a.exp){
                        const c = new Date(a.exp * 1e3), d = c < new Date;r += `
    ...
    `).join(''), a = document.getElementById('issuer-metrics-list');if (a && (a.innerHTML = r || '<div class="no-data">No issuer data available</div>'), e.recentErrors.length > 0){
                const c = e.recentErrors.map(m => `
        ...
        `).join(''), d = document.getElementById('error-metrics-list');d && (d.innerHTML = c);
            } else { const c = document.getElementById('error-metrics-list');c && (c.innerHTML = '<div class="no-errors">No recent errors</div>'); }
        }, Zt = () => {
            const e = document.getElementById('jwt-metrics-content');e && (e.innerHTML = `
    ...
    `, t = document.getElementById('help');if (L.info('Help tab pane found:', !!t), t)L.info('Appending help content to tab pane'), t.innerHTML = e, L.info('Help content appended, new length:', t.innerHTML.length);else { L.warn('Help tab pane not found, appending to container');const s = document.getElementById('jwt-validator-container');s && s.insertAdjacentHTML('beforeend', e); }
        }, is = () => { document.querySelectorAll('.collapsible-header').forEach(e => { e.addEventListener('click', function (){ const t = this.nextElementSibling, s = this.querySelector('i.fa');this.classList.toggle('active'), t && t.classList.contains('collapsible-content') && t.classList.toggle('show'), s && (this.classList.contains('active') ? (s.classList.remove('fa-chevron-right'), s.classList.add('fa-chevron-down')) : (s.classList.remove('fa-chevron-down'), s.classList.add('fa-chevron-right'))), L.debug('Toggled help section:', this.textContent.trim()); }); }); }, De = Object.freeze(Object.defineProperty({ __proto__: null, cleanup: () => { L.debug('Cleaning up help tab'); }, getDisplayName: () => g.getI18n().getProperty(C.I18N_KEYS.HELP_TAB_NAME) || 'Help', init: ne }, Symbol.toStringTag, { value: 'Module' }));let He = 'en';const $e = ['en', 'de'], ns = function (){ const t = (navigator.language || navigator.userLanguage || 'en').split('-')[0];return $e.includes(t) ? t : 'en'; }, os = function (e){ return $e.includes(e) ? (He = e, !0) : !1; }, rs = function (){ return He; };os(ns());function H(e, t = {}, s = document){ let r;if (e == null) return null;let i;if (typeof e == 'string' ? i = s.querySelectorAll(e) : Array.isArray(e) ? i = e : i = [e], i.length === 0) return null;const o = { ...{ placement: 'bottom-start', arrow: !0, theme: 'light-border', appendTo: 'parent' }, ...t };try { return Xe(Array.from(i), o); } catch (a){ return (r = g == null ? void 0 : g.logError) == null || r.call(g, 'Error initializing tooltip: ' + a.message), null; } } const as = { 'ctrl+enter': 'verify-token', 'cmd+enter': 'verify-token', 'alt+v': 'verify-token', 'ctrl+1': 'goto-tab-1', 'ctrl+2': 'goto-tab-2', 'ctrl+3': 'goto-tab-3', 'cmd+1': 'goto-tab-1', 'cmd+2': 'goto-tab-2', 'cmd+3': 'goto-tab-3', 'ctrl+s': 'save-form', 'cmd+s': 'save-form', 'alt+r': 'reset-form', escape: 'close-dialog', f1: 'show-help', '?': 'show-help' }, Fe = new Map, Be = () => { We(), document.addEventListener('keydown', ze), window.__keyboardShortcutHandler = ze, gs(); }, ze = e => { const t = Je(e), s = as[t];s && cs(e) && (e.preventDefault(), e.stopPropagation(), ls(s)); }, Je = e => { const t = [];(e.ctrlKey || e.metaKey) && t.push(e.ctrlKey ? 'ctrl' : 'cmd'), e.altKey && t.push('alt'), e.shiftKey && t.push('shift');const s = e.key ? e.key.toLowerCase() : '';return s === 'enter' ? t.push('enter') : s === 'escape' ? t.push('escape') : s === 'f1' ? t.push('f1') : s && s.match(/^[a-z0-9?]$/) && t.push(s), t.join('+'); }, cs = e => { const t = e.target, s = t.tagName ? t.tagName.toLowerCase() : '';if (s === 'input' || s === 'textarea'){ const i = Je(e);return i === 'ctrl+enter' || i === 'cmd+enter' || i === 'escape'; } return !0; }, ls = e => { if (e != null && e.startsWith('custom-')){ const t = Fe.get(e);if (t && t.handler){ t.handler();return; } } switch (e){ case 'verify-token':ds();break;case 'goto-tab-1':case 'goto-tab-2':case 'goto-tab-3':us(parseInt(e.split('-')[2]) - 1);break;case 'save-form':ps();break;case 'reset-form':fs();break;case 'close-dialog':ms();break;case 'show-help':hs();break;default:console.debug('Unknown keyboard shortcut action:', e); } }, ds = () => { const e = document.querySelector('.verify-token-button:not(:disabled)');e && e.offsetParent !== null && (e.click(), $('Token verification started')); }, us = e => { const t = document.querySelectorAll('.tab-nav-item');t.length > e && (t[e].click(), $(`Switched to tab ${e + 1}`)); }, ps = () => { const t = Array.from(document.querySelectorAll('button:not(:disabled)')).find(s => s.offsetParent !== null && (s.textContent.includes('Save') || s.textContent.includes('Apply')));t && (t.click(), $('Form save triggered')); }, fs = () => { const t = Array.from(document.querySelectorAll('button:not(:disabled)')).find(s => s.offsetParent !== null && (s.textContent.includes('Reset') || s.textContent.includes('Clear')));t && (t.click(), $('Form reset triggered')); }, ms = () => { const e = document.querySelector('.ui-dialog-titlebar-close') || Array.from(document.querySelectorAll('button')).find(t => t.offsetParent !== null && (t.textContent.includes('Cancel') || t.textContent.includes('Close')));e && (e.click(), $('Dialog closed')); }, hs = () => {
            const t = `
