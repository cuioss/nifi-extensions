...
describe('updateComponentProperties', () => {
    ...
    // Settle-at-entry: a processor caught mid-restart reports a TRANSITIONAL
    // state (Starting/STARTING) rather than a settled Running/RUNNING. A prior
    // mutation returns without awaiting its restart, so the next mutation can
    // begin while that restart is in flight. updateProcessorWithStopStart must
    // wait for the processor to settle to a STABLE state (here RUNNING) at its
    // ENTRY before issuing any stop/PUT — otherwise the stop/PUT races the
    // in-flight restart and NiFi rejects it ("Cannot modify configuration while
    // the Processor is running").
    test.each([
        ['runStatus Starting', { status: { runStatus: 'Starting' } }],
        ['state STARTING', { state: 'STARTING' }]
    ])('should wait for a transitional processor (%s) to settle before stop/PUT', async (_label, transitionalState) => {
        const procId = '00000000-0000-0000-0000-000000000012';
        globalThis.jwtAuthConfig = { processorId: procId };
        // Detection
        mockJsonResponse({ type: 'PROCESSOR', componentClass: 'SomeProcessor' });
        // GET #1 at entry — mid-transition (restart still in flight); NOT stable
        mockJsonResponse({ revision: { version: 1 }, component: { id: procId, ...transitionalState, config: { autoTerminatedRelationships: [] } } });
        // GET #2 at entry — now settled to a stable RUNNING state
        mockJsonResponse({ revision: { version: 1 }, component: { id: procId, state: 'RUNNING', config: { autoTerminatedRelationships: [] } } });
        // PUT stop
        mockJsonResponse({ revision: { version: 2 } });
        // GET poll — STOPPED
        ...
        // The entry settle re-GETs the processor until it is stable: two GETs to
        // the processor URL precede any stop/PUT (call[1] transitional, call[2]
        // settled), and no stop/PUT is issued against the transitional read.
        const entryGet1 = globalThis.fetch.mock.calls[1];
        const entryGet2 = globalThis.fetch.mock.calls[2];
        expect(entryGet1[0]).toBe(`/nifi-api/processors/${procId}`);
        expect(entryGet1[1].method).toBe('GET');
        expect(entryGet2[0]).toBe(`/nifi-api/processors/${procId}`);
        expect(entryGet2[1].method).toBe('GET');

        // Only after settling is the stop issued...
        const stopCall = globalThis.fetch.mock.calls[3];
        expect(stopCall[0]).toBe(`/nifi-api/processors/${procId}/run-status`);
        expect(stopCall[1].method).toBe('PUT');
        expect(JSON.parse(stopCall[1].body).state).toBe('STOPPED');

        // ...and only then does the property PUT go out.
        const updateCall = globalThis.fetch.mock.calls[6];
        expect(updateCall[0]).toBe(`/nifi-api/processors/${procId}`);
        expect(updateCall[1].method).toBe('PUT');
        expect(JSON.parse(updateCall[1].body).component.config.properties.key).toBe('value');

        // It settled to RUNNING, so it is restarted afterwards.
        const restartCall = globalThis.fetch.mock.calls[9];
        expect(restartCall[0]).toBe(`/nifi-api/processors/${procId}/run-status`);
        expect(JSON.parse(restartCall[1].body).state).toBe('RUNNING');
    });

    // Common case: a processor already in a stable state must NOT be slowed by
    // the settle-at-entry poll — the entry reads run state with a single GET and
    // proceeds straight to the stop/PUT, adding no extra round-trip.
    test('should not add a settling GET when the processor is already stable', async () => {
        const procId = '00000000-0000-0000-0000-000000000013';
        globalThis.jwtAuthConfig = { processorId: procId };
        // Detection
        mockJsonResponse({ type: 'PROCESSOR', componentClass: 'SomeProcessor' });
        // GET at entry — already stable RUNNING (settles on the first GET)
        mockJsonResponse({ revision: { version: 1 }, component: { id: procId, state: 'RUNNING', config: { autoTerminatedRelationships: [] } } });
        // PUT stop
        mockJsonResponse({ revision: { version: 2 } });
        // GET poll — STOPPED
        ...
        // Exactly one GET at entry (call[1]); the stop PUT is call[2], not a
        // second settling GET — the stable common case incurs no extra round-trip.
        const entryGet = globalThis.fetch.mock.calls[1];
        expect(entryGet[0]).toBe(`/nifi-api/processors/${procId}`);
        expect(entryGet[1].method).toBe('GET');
        const stopCall = globalThis.fetch.mock.calls[2];
        expect(stopCall[0]).toBe(`/nifi-api/processors/${procId}/run-status`);
        expect(stopCall[1].method).toBe('PUT');
        expect(JSON.parse(stopCall[1].body).state).toBe('STOPPED');
