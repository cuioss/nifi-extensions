...
NiFi loads a pre-configured flow on startup (`flow.json`, gzipped as `flow.json.gz`).
...
The full flow has 11 processors. The extra steps exist so NiFi can extract the bearer token, set the response status code, log what happened, and return jwt attributes as JSON:

. **HandleHttpRequest** -- accepts HTTP on port 7777
. **UpdateAttribute** (Extract Token) -- extracts the bearer token from the `Authorization` header into the `jwt.token` attribute
...
----
{
  ...
  "jwt.issuer": "https://keycloak:8443/realms/oauth_integration_tests",
  "jwt.roles": "read,user",
...
- JWKS URL: `https://keycloak:8443/realms/oauth_integration_tests/protocol/openid-connect/certs`
- Issuer: `https://keycloak:8443/realms/oauth_integration_tests`
- Required role: `read`
- HTTPS for JWKS: required (`jwt.validation.require.https.for.jwks=true`); private-network addresses are allowed
...
Client: `test_client` (public client -- no secret required)
...
|No Authorization header |401 |`jwt.error.category=EXTRACTION_ERROR`, `jwt.error.code=AUTH-001`
...
- Uses a truststore-based SSL context for Docker's self-signed certificates (validates certificates against the known CA)
...
TOKEN=$(curl -sk -X POST https://localhost:9085/realms/oauth_integration_tests/protocol/openid-connect/token \
...
gzip -c flow.json > flow.json.gz
----

Restart the NiFi container after changes.
