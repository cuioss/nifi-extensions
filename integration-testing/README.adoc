...
----
src/main/docker/
├── Dockerfile                     # NiFi container (based on apache/nifi:2.4.0)
...
│   ├── flow.json / flow.json.gz / flow.xml.gz  #   Pre-configured flow (JSON source + gzipped)
...
│   ├── wait-for-containers.sh    #   Health polling: containers → NiFi UI → flow pipeline
...
|Port conflict on 9085, 9086, or 9095
